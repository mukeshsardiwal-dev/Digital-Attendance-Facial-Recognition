...
async function captureImagesLoop() {
  ...
  console.log(
    `Uploading ${images.length} images for student_id: ${student_id}`,
  );
  ...
  console.log("Upload response:", respData);

  if (resp.ok && respData.saved > 0) {
    alert(
      `Uploaded ${respData.saved} images successfully. Now click 'Add Student' to proceed to training.`,
    );
    addStudentBtn.disabled = false;
  } else {
    alert(
      `Upload failed or no images saved. Response: ${JSON.stringify(respData)}`,
    );
  }
