...
def train_model_route():
    ...
    # Check if any student has images before training
    student_dirs = [
        d
...
# -------- Debug: Check dataset status --------
@app.route("/debug/check_dataset", methods=["GET"])
def check_dataset():
    """Debug endpoint to verify dataset structure and training readiness"""
    ...
    # Ready to train if at least one student with images
...
# -------- Cleanup: Remove empty student folders --------
@app.route("/cleanup_empty_students", methods=["POST"])
def cleanup_empty_students():
    """Remove student folders that have no images"""
    student_dirs = [
        d
        for d in os.listdir(DATASET_DIR)
        if os.path.isdir(os.path.join(DATASET_DIR, d))
